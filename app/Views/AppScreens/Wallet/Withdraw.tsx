...
const Withdraw = () => {
  const { selectedCoin, minedAmount } = useSelector((state: RootState) => state.coin);
  ...
  const validate = () => {
    ...
    if (!amount || isNaN(Number(amount)) || Number(amount) <= 0) {
      ...
      isValid = false;
    } else if (Number(amount) > minedAmount) {
      newError.amount = 'Insufficient balance';
      isValid = false;
    }
  ...
  const handleWithdraw = () => {
    if (validate()) {
      console.log(`Withdrawing ${amount} ${selectedCoin} to ${walletAddress}`);
